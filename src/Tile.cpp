...
#include <stdexcept>
#include <algorithm>
#include <cctype> // Để sử dụng toupper và tolower

// Triển khai DEFAULT_SCORES với bảng điểm Scrabble tiêu chuẩn, bao gồm blank tile
const std::map<char, int> Tile::DEFAULT_SCORES = {
    ...
    {'U', 1}, {'V', 4}, {'W', 4}, {'X', 8}, {'Y', 4},
    {'Z', 10}, {' ', 0} // Thêm điểm cho blank tile
};
...
Tile::Tile(char letter, bool isBlank)
    ...
    if (isBlank) {
        value_ = 0; // Blank tile có điểm 0
        letter_ = ' '; // Đặt letter về khoảng trắng
    } else {
...
void Tile::setValue(int value) {
    value_ = std::max(0, value); // Đảm bảo điểm không âm
}
