...
void TrieDictionary::find_matches_recursive(
    ...
    if (!node) {
        if (pattern_index >= pattern.size() && node && node->is_end_of_word) {
             results.push_back(current_word);
        }
        return;
    }
    if(pattern_index >= pattern.size()){
        if(node->is_end_of_word) results.push_back(current_word);
        return;
    }
...
void TrieDictionary::find_possible_words_recursive(
    ...
    for (const auto& [ch, child] : node->children) {
        if (available_letters.count(ch) && available_letters[ch] > 0) {
